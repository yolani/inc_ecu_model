...
links elements the same way: a reference field holds either the referenced
element directly or an unresolved `Identifier` / `FullyQualifiedName` until
resolution replaces it. `ModelRegistry` still tracks every element under its
`id` for lookup and process-wide queries (e.g. "find all enums"), but resolved
references themselves are plain object references, not a UUID indirection.

Prefer creating model elements before creating the model objects that contain
references to them, and pass elements directly at construction sites. Where
that is not possible, e.g. for circular imports, pass the name and replace it
...
| `namespace` | `namespace` | Convert `BaseQualifiedName` to a string using the selected source-language separator. Do not use Orion's unconditional `::` key format. Assignable after construction for producers that learn the namespace late. |
...
Each Orion `BaseQualifiedName | DataTypeDefinition` link maps to SCORE's
`DataTypeOrReference`, which accepts a `PrimitiveDataType` member, a declared
`DataTypeBase` element, or an unresolved `Identifier` / `FullyQualifiedName`.
`extends` accepts a declared element or an unresolved reference.
...
Prefer resolving a qualified-name link to exactly one declared SCORE data type
and passing that element, building declarations in dependency order (leaves
first). When the target cannot exist yet, keep the name in the field and assign
the resolved element later:

```python
alias = TypedefDataType(
    identifier="PositionAlias",
    source_kind=DataTypeSource.FRANCA,
    data_type=FullyQualifiedName(
        identifier=Identifier("Position"),
        namespace=(Identifier("app"), Identifier("geometry")),
    ),
)
alias.data_type = position
```

A plain string is validated as an unresolved reference, so primitives must be
passed as `PrimitiveDataType` members. A model whose reference fields still hold
names is not fully resolved; report missing or ambiguous names before handing
the model to downstream tools.
...
4. Create declared types, passing referenced targets directly where dependency
   order allows, and the referenced name otherwise.
5. Resolve remaining qualified-name links against the builder's declaration
   index and assign the resolved elements; report missing or ambiguous names
   before model construction completes.
...
Enum values and composite fields are validated immediately and must therefore
exist before their containing enum, struct, or union is built.
...
- Struct and union field identifiers must be unique; explicit field numbers must
  be unique.
- `extends` is allowed only for Franca and, once resolved, must refer to the
  same data-type kind.
